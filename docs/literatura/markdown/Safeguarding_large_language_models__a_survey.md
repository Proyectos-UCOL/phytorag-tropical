Received: 21 October 2024 / Accepted: 31 August 2025 / Published online: 17 October 2025
© The Author(s) 2025
...
1	
Department of Computer Science, University of Liverpool, Brownlow Street, 
...
the requirements need to be precisely defined, and their corresponding metrics, and a multidisciplinary approach is called for. Mitigating a given requirement (such as hallucinations, 
...
and guidelines that meet requirements for applications and users. This is a Type 1 neuralsymbolic system (Lamb et al. 2021), i.e., typical deep learning methods where the input and 
...
lucination prevention in responses, and content moderation. NeMo is also a Type-1 neuralsymbolic system, with its effectiveness closely tied to the performance of the KNN method.
...
RAG triad, including context relevance, answer relevance, and groundedness. RetrievalAugmented Generation (RAG) is a technology that enhances the quality of language model 
...
language, assuring the variable insertion in any prompts. The Guidance program has a welldefined linear execution order directly corresponding to the token sequence processed by 
...
solutions. (2) AI Fairness 360 (AIF360)5: AIF360 is an extended toolkit from IBM that 
...
rity of machine learning applications. (3) Adversarial Robustness Toolbox (ART)6: ART 
...
The practical application examples9 are shown in Table 2, which documents real-world 
...
Content Moderation
(Bodhankar 2024; Singh 2024)
(Bodhankar 2024; Singh 2024)
(Team 2024)
(Team 2023)
(team 2023) (Lab 2023)

Cyber Treats Defense
(Singh 2024; Wan et al. 2024;
...
(Team 2024)
–
(team 2023) (Beurer-Kellner et al.
...
or fine-tuned on adversarial datasets (Barrantes et al. 2020). Nowadays, the instructionfollowing abilities of LLMs are leveraged for autonomous evaluation. LLMs can effectively 
...
have censorship systems that can filter out content violating legal standards in realtime, ensuring a seamless user experience. However, research indicates that seman­
...
fundamental method involves a semantic equivalence relation, denoted as E(si, sj), where 
...
White
User
Greedy Gradient-based Search
Low
×
Harmful Content

PGD (Geisler et al. 2024)
White
User
Continuous Relaxation & Entropy
...
PRP* (Mangaokar et al. 2024)
White
User
In-context Learning & two-step
...
JailBroken (Wei et al. 2024)
Black
System
Failure modes as guiding principles
Low
✓
Harmful Content & personally

identifiable information leakage

DeepInception (Li et al. 2023)
Black
User
Nested instruction
Medium
...
Black
User
Characterizing in-the-wild prompt
High
✓
Harmful Content
...
Black
User
In-context learning ability of LLM
Low
×
Harmful Content
...
Mosaic Prompts (Glukhov et al. 2023)
Black
User
Semantic censorship
High
...
Black
System+User
Unified framework for 12 jailbreaks
-
✓
...
Black
User
Mask-based iterative strategy
Low
×
Goal hijacking & prompt leaking

IPI (Greshake et al. 2023)
Black
System
Indirect prompt injection
High
✓
Cyber threats like theft of data and

denial of service etc

HOUYI (Liu et al. 2023)
Black
User
SQL injection & XSS attacks
Low
×
Prompt abuse & prompt leak
...
Pelrine et al. (Pelrine et al. 2023) *
Grey
User
fine-tuning
Low
✓
Misinformation & Privacy Leakage

Zhang et al. (Zhan et al. 2023)
Grey
User
fine-tuning
Low
...
Safety-tuned (Bianchi et al. 2023)
Grey
User
fine-tuning
Low
...
Janus inference (Chen et al. 2023)
Grey
System
Fine-tuning
Low
...
Grey
User
poisoning knowledge retrieval
Medium
✓
Harmful content & fairness
...
ActivationAttack (Wang and Shu 2023)
Grey
System
Activation Steering
Low
...
characters that are easily detectable by simple perplexity filter (Jain et al. 2023). AutoDANZhu (Zhu et al. 2023) design a double-loop optimization method upon GCG to generate 
...
Unlike white-box attacks, which necessitate access to model weights and tokenizers, blackbox attacks operate under the premise that adversaries lack knowledge of the LLM’s internal 
...
et al. (2023) show that instruction-following language models using the text-davinci-003 
...
and responses to refine based on a judge score and generate new prompts. Through incontext learning, PAIR enabled the language model to enhance the quality of generated 
...
vulnerabilities in the safeguard systems of LLMs. This section will present studies on ‘greybox’ attack methods, encompassing strategies like fine-tuning, retrieval-augmented genera­
...
behaviors in downstream models. Shah et al. (2023) introduces LoFT (Local Proxy Finetuning) to fine-tuning smaller, local proxy models to develop attacks that are more likely 
...
with safety standards based on these adversarial prompts. Through several rounds of redteaming, the enhanced target LLM continues to bolster its defenses through safety-specific 
...
and it has been a trend to consider domain-specific LLMs (Pal et al. 2023). In domainspecific scenarios, specialized rules may conflict with the general principles. For instance, 
...
scientifically determine requirements. The above challenges (multiple categories, domainspecific, and potentially conflicting requirements) are compounded by the fact that many 
...
For the evaluation of multiple, conflicting requirements, a combination of the Pareto frontbased evaluation methods for multiple requirements (Ngatchou et al. 2005) and the statisti­
...
As multimodal large language models (MLLMs) become increasingly prominent in realworld applications, ranging from AI assistants with visual perception to audio-driven 
...
safety evaluation system built upon LLaVA, capable of policy-aware classification of image 
...
OpenLM Research
3B-13B
Open-source replication of Meta’s 
LLaMA models
Open-Weight (M)LLMs
 LLaMA 2
Meta
...
A, Agarwal S, Herbert-Voss A, Krueger G, Henighan T, Child R, Ramesh A, Ziegler DM, Wu J, Winter 
...
8​6​5​3​/​v​1​/​2​0​2​0​.​f​i​n​d​i​n​g​s​-​e​m​n​l​p​.​3​0​1
...
t​p​s​:​/​/​b​l​o​g​.​f​l​i​p​k​a​r​t​.​t​e​c​h​/​f​l​i​p​k​a​r​t​-​e​n​h​a​n​c​e​s​-​a​i​-​s​a​f​e​t​y​-​i​n​-​e​-​c​o​m​m​e​r​c​e​-​i​m​p​l​e​m​e​n​t​i​n​g​-​n​v​i​d​i​a​-​n​e​m​o​-​g​u​a​r​d​r​a​
